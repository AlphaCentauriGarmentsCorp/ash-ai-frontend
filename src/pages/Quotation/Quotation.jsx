...
const Quotation = () => {
  ...
  const [colorCount, setColorCount] = useState(null); // Start as null
  const [discount, setDiscount] = useState({ type: "percentage", value: 0 });

  // Selected IDs for global config - start as null
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Don't initialize items until user selects options
      setItems([]);
    } catch (error) {
  ...
  useEffect(() => {
    if (
      ...
      // Only initialize if items is empty or if we're resetting
      if (items.length === 0) {
  ...
  return (
    ...
    >
      <section className="flex flex-col gap-y-3 sm:gap-y-4 font-poppins p-3 sm:p-4 max-w-full mx-auto bg-light rounded-lg border border-gray-300">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-1 gap-3.75">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setOrderInfo({ ...orderInfo, client_email: e.target.value })
                  }
                  className="w-full px-2 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-primary/20 focus:border-primary"
        ...
        {/* Quotation Items Table - Only show if selections are made */}
        {hasSelections && (
          <>
            ...
            <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
              <div className="px-4 py-3 bg-primary/5">
                <h3 className="text-sm font-semibold text-primary">
                  <i className="fas fa-plus-circle mr-2"></i>Addons (Applied to
                  all sizes)
                </h3>
              </div>
              <div className="p-3">
            ...
            {/* Cost Breakdown, Discount Section, Payment Summary, Notes, and Action Buttons */}
                            ...
                            {itemDetails.map((item) => {
                              const sizeName =
                                data.sizes.find((s) => s.id === item.size_id)
                                  ?.name || "Unknown";
                              const tshirtPrice =
                                quotationService.getTshirtPrice(
                                  data.tshirtTypes,
                                  item.tshirt_type_id,
                                );
                              const sizePrice = quotationService.getSizePrice(
                                data.sizePrices,
                                item.tshirt_type_id,
                                item.size_id,
                              );
                              const necklinePrice =
                                quotationService.getNecklinePrice(
                                  data.necklines,
                                  item.neckline_id,
                                );
                              const printTypePrice =
                                quotationService.getPrintTypePrice(
                                  data.printTypes,
                                  item.print_type_id,
                                );
                              const printColorPrice =
                                quotationService.getPrintColorPrice(
                                  data.printColors,
                                  item.print_type_id,
                                  colorCount || 1,
                                );
                              const printPatternPrice =
                                quotationService.getPrintPatternPrice(
                                  data.printPatterns,
                  ...
                  <div className="mt-4 pt-2 border-t-2 border-primary/20">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-xl font-bold text-primary">
                        ₱{(totalAmount + totalAddons).toLocaleString()}
                      </span>
                    </div>
                  </div>
                </div>
        ...
        {/* Show message when no selections are made */}
        {!hasSelections && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-center">
            <i className="fas fa-info-circle text-yellow-600 mr-2"></i>
            <span className="text-sm text-yellow-700">
              Please select all configuration options above to start creating
              your quotation.
            </span>
          </div>
        )}
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <button
            onClick={handleSave}
            className="px-3 py-1.5 rounded-md bg-primary text-white hover:bg-secondary text-xs"
          >
            <i className="fas fa-save mr-1"></i>Save Quotation
          </button>
        </div>
      </section>
    </AdminLayout>
