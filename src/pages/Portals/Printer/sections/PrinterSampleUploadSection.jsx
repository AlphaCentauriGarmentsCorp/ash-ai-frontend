...
/**
 * Identical to Cutter's SampleUploadSection in behavior, but routes
 * writes through printerPortalApi (which hits /portal/printer/sample-uploads).
 *
 * Keeping a separate component (instead of parameterizing) keeps each
 * portal's section self-contained and easy to customize per role.
 */
...
const PrinterSampleUploadSection = ({
  sampleUploads = [],
  orderId,
  orderStageId,
  onChanged,
  ...
  const handleSubmit = async (markDone) => {
    ...
    try {
      await printerPortalApi.createSampleUpload(
        {
          order_id: orderId,
    ...
    <section className="bg-white rounded-lg border border-gray-200 p-5">
      ...
      <div className="mb-3">
        ...
        <textarea
          ...
          placeholder="Hal. Okay ang print sa front, likod medyo light."
          disabled={submitting}
              ...
              >
                <div className="flex gap-1">
                  {u.photo_front_url ? (
                    ...
                    <div className="w-14 h-14 rounded border border-dashed border-gray-200 flex items-center justify-center text-gray-300 text-xs">
                      F
                    </div>
                  )}
                  {u.photo_back_url ? (
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <span
                      className={`text-[10px] capitalize px-2 py-0.5 rounded ${
                        SAMPLE_STATUS_STYLES[u.sample_status] ||
                        SAMPLE_STATUS_STYLES.pending
                      }`}
                    >
...
export default PrinterSampleUploadSection;
