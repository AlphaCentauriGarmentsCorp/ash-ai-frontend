...
const AddApparelType = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await apparelTypeApi.create(formData);
    ...
    setTimeout(() => {
      ...
      if (err.response?.data?.errors) {
        ...
        setServerError(
          err.response?.data?.message || "Failed to create apparel type.",
        );
      }
    } finally {
      setIsSubmitting(false);
  ...
  return (
    ...
    >
      {submitSuccess && (
          <AlertMessage
            type="success"
            title="Apparel Type created successfully!"
            message="The new apparel type has been added to the system."
          />
        )}
        ...
        {submitSuccess && (
          <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-md">
            <p className="text-green-800 font-medium">
              Apparel Type created successfully!
            </p>
          </div>
        )}
