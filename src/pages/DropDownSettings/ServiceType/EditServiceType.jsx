...
const EditServiceType = () => {
  ...
  const fetchServiceType = async () => {
    try {
      const response = await serviceTypeApi.show(id);
      setFormData(response.data);
    } catch (error) {
      setServerError("Failed to load service type.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      await serviceTypeApi.update(id, formData);
      setSubmitSuccess(true);

      window.scrollTo({ top: 0, behavior: "smooth" });

      setTimeout(() => {
        navigate("/admin/settings/service-type");
      }, 1500);
    } catch (error) {
      setServerError("Failed to update service type.");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <AdminLayout
      ...
      links={[
        ...
        { label: "Service Type", href: "/admin/settings/service-type" },
      ]}
    >
