...
const ServiceTypePage = () => {
  ...
  const fetchData = useCallback(
    async (perPage = pageSize) => {
      ...
      try {
        ...
        if (perPage === "all") {
          response = await serviceTypeApi.index();
        } else {
          response = await serviceTypeApi.index({ per_page: perPage });
        }
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true);
    try {
      await serviceTypeApi.delete(selectedItem.id);
      setData((prev) => prev.filter((item) => item.id !== selectedItem.id));
      setIsDeleteDialogOpen(false);
    } catch (error) {
      await alert({
        title: "Couldn't delete service type",
  ...
  const handleAction = (action, rowData) => {
    switch (action) {
      case "edit":
        navigate(`/admin/settings/service-type/edit/${rowData.id}`);
        break;
      case "delete":
  ...
  return (
    ...
    >
      <Table
        ...
        isLoading={isLoading}
        url="/admin/settings/service-type/new"
