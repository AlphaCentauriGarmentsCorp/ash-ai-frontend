...
const CourierPage = () => {
  ...
  const fetchData = useCallback(
    async (perPage = pageSize) => {
      ...
      try {
        ...
        if (perPage === "all") {
          response = await courierApi.index();
        } else {
          response = await courierApi.index({ per_page: perPage });
        }
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true);
    try {
      await courierApi.delete(selectedItem.id);
  ...
  const handleAction = (action, rowData) => {
    switch (action) {
      case "edit":
        navigate(`/admin/settings/courier/edit/${rowData.id}`);
        break;
      case "delete":
  ...
  return (
    ...
    >
      <Table
        ...
        isLoading={isLoading}
        url="/admin/settings/courier/new"
        button="Add Courier"
        PageTitle="Courier"
      />
