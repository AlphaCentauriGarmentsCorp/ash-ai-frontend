...
const EditCourier = () => {
  ...
  const fetchCourier = async () => {
    try {
      const response = await courierApi.show(id);
      setFormData(response.data);
    } catch (error) {
      setServerError("Failed to load courier.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      await courierApi.update(id, formData);
      setSubmitSuccess(true);

      window.scrollTo({ top: 0, behavior: "smooth" });

      setTimeout(() => {
        navigate("/admin/settings/courier");
      }, 1500);
    } catch (err) {
      if (err.response?.data?.errors) {
        ...
        setServerError(
          err.response?.data?.message || "Failed to edit courier.",
        );
      }
    } finally {
      setIsSubmitting(false);
  ...
  if (isLoading) {
    return (
      <AdminLayout pageTitle="Edit Courier">
  ...
  return (
    <AdminLayout
      ...
      links={[
        ...
        { label: "Courier", href: "/admin/settings/courier" },
      ]}
    >
