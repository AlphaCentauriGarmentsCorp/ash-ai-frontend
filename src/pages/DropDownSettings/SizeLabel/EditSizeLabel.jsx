...
const EditSizeLabel = () => {
  ...
  const fetchSizeLabel = async () => {
    try {
      const response = await sizeLabelApi.show(id);
      setFormData(response.data);
    } catch (error) {
      setServerError("Failed to load size label.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      await sizeLabelApi.update(id, formData);
  ...
  if (isLoading) {
    return (
      <AdminLayout pageTitle="Edit Size Label">
