...
const EditShippingMethods = () => {
  ...
  const fetchShippingMethod = async () => {
    try {
      const response = await shippingMethodsApi.show(id);
      setFormData(response.data);
    } catch (error) {
      setServerError("Failed to load shipping method.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      await shippingMethodsApi.update(id, formData);
      ...
      if (err.response?.data?.errors) {
        ...
        setServerError(
          err.response?.data?.message || "Failed to edit shipping method.",
        );
      }
    } finally {
      setIsSubmitting(false);
  ...
  if (isLoading) {
    return (
      <AdminLayout pageTitle="Edit Shipping Method">
