...
const AddAddonCategories = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await addonCategoriesApi.create(formData);
      ...
      if (err.response?.data?.errors) {
        ...
        setServerError(
          err.response?.data?.message || "Failed to create addon categories.",
        );
      }
    } finally {
      setIsSubmitting(false);
  ...
  return (
    <AdminLayout
      pageTitle="Add Addon Categories"
    ...
    >
      <div className="bg-light p-3 lg:p-7 rounded-lg border border-gray-300">
        {submitSuccess && (
          <AlertMessage
            type="success"
            title="Addon Categories created successfully!"
            message="The new addon categories has been added to the system."
          />
        )}
