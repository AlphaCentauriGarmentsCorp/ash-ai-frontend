...
const AddPrintTypes = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await printTypesApi.create(formData);
      ...
      if (err.response?.data?.errors) {
        ...
        setServerError(
          err.response?.data?.message || "Failed to create print types.",
        );
      }
    } finally {
      setIsSubmitting(false);
    ...
    >
      <div className="bg-light p-3 lg:p-7 rounded-lg border border-gray-300">
        {submitSuccess && (
          <AlertMessage
            type="success"
            title="Print Types created successfully!"
            message="The new print types has been added to the system."
          />
        )}
