...
const AddApparelPatternPrices = () => {
  ...
  useEffect(() => {
    const fetchDropdowns = async () => {
      try {
        const [apparelRes, patternRes, existingRes] = await Promise.all([
  ...
  const handleSubmit = async () => {
    ...
    try {
      await apparelPatternPricesApi.create({
      ...
      setErrors({});
      setFormData(apparelPatternPricesInitialState);
      window.scrollTo({ top: 0, behavior: "smooth" });
      ...
      if (err.response?.data?.errors) {
        ...
        setServerError(
          err.response?.data?.message ||
            "Failed to create apparel pattern price.",
        );
      }
    } finally {
      setIsSubmitting(false);
  ...
  return (
    <AdminLayout
      pageTitle="Add Apparel Pattern Prices"
    ...
    >
      <div className="bg-light p-3 lg:p-7 rounded-lg border border-gray-300">
        {submitSuccess && (
          <AlertMessage
            type="success"
            title="Apparel pattern price created successfully!"
            message="The new apparel pattern price has been added to the system."
          />
        )}
...
export default AddApparelPatternPrices;
